// src/components/PitchTrainer.tsx
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import { Button, Title, Text, Card, Group, Badge, Stack, Progress, ThemeIcon, Select, Checkbox } from '@mantine/core';
...
export function PitchTrainer() {
  ...
  const [showTargetAlways, setShowTargetAlways] = useState(false); // New state for checkbox

  // Real-time Feedback
  const successStreak = useRef(0); // How many consecutive frames correct?
  const REQUIRED_STREAK = 1; // approx 150-200ms depending on websocket rate
  ...
  const socketUrl = SAMPLE_RATE_URL(sampleRate);
  ...
  return (
    <Card shadow="lg" padding="xl" radius="lg" withBorder>
        ...
        {gameState !== 'idle' && (
          <Stack mb="xl" align="center">
             <Fretboard
                ...
                targetNote={target}
                // Only show the highlight if they got it right
                showHighlight={showTargetAlways || gameState === 'success'}
             />
          </Stack>
        )}
        {/* --- TUNING METER --- */}
        ...
        <Checkbox
          label="Always show target note"
          checked={showTargetAlways}
          onChange={(event) => setShowTargetAlways(event.currentTarget.checked)}
          mb="xl"
          disabled={isListening}
        />
